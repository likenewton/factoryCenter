    ...
    <el-card class="clearfix" shadow="never" v-loading="loadData">
      <el-row>
        <el-form :inline="true" :model="formInline" class="search-form" size="small" @submit.native.prevent>
          ...
          <el-form-item>
            <el-button type="primary" @click="simpleSearch('factoryName')" :disabled="!pageAuthBtn.factory_package_list">查询</el-button>
            <el-button type="primary" @click="searchVipVisible = true" :disabled="!pageAuthBtn.factory_package_list">高级查询</el-button>
          </el-form-item>
        </el-form>
      </el-row>
      <el-row>
...
export default {
  ...
  methods: {
    ...
    goDetail(row) {
      let factoryName = Api.UNITS.valueToLabel(row.factoryName, this.yunovoDic.filter((v) => v.wordType === 1), 'wordValue', 'wordKey')
      this.$router.push({
        name: 'patchDetail',
        query: {
